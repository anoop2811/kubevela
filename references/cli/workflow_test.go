...
func TestWorkflowList(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.typeFlag != "" {
				args = append(args, "-t", tc.typeFlag)
			}
			...
			for i, line := range lines[1:] {
				...
				if workflow, ok := tc.workflows[i].(*workflowv1alpha1.WorkflowRun); ok {
					if tc.namespace == "" {
						r.Equal(fields[0+offset], workflow.Namespace)
						offset = offset + 1
					} else if tc.namespace == workflow.Namespace && (tc.typeFlag != "" && tc.typeFlag == "WorfklowRun") {
					...
					if tc.namespace == "" {
						...
						offset = offset + 1
					} else if tc.namespace == app.Namespace && tc.typeFlag != "" && tc.typeFlag == "Application" {
